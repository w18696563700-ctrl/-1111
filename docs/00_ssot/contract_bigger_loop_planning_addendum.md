...
owner: Codex 总控
status: draft
purpose: Freeze the planning-only boundary for Contract bigger-loop discussion after the minimum contract loop has already been closed.
layer: L0 SSOT
---

# Contract bigger-loop planning 补充单

## Scope
- This addendum applies only to `Contract bigger-loop planning`.
- It is a planning-truth freeze only.
- It does not unlock implementation by itself.
- It does not reopen the already closed minimum `Contract` loop by itself.

## Canonical Decisions

### 1. Current closure baseline remains valid
- The already completed minimum `Contract` loop remains the current canonical
  closed baseline:
...
- Outside the minimum loop, the allowed `Contract bigger-loop` planning range is
  limited to whether a later round should discuss:
  - clause-structure and clause-editing boundary only
  - signing workflow boundary only
  - legal-review or collaboration boundary only
  - history, list, or reporting boundary only
  - governance or operations visibility boundary only
...
- This round does not approve:
  - clause editor implementation
  - sign workflow implementation
  - legal review implementation
  - contract history or list implementation
...
- This means the round may freeze only:
  - what `Contract bigger-loop planning` is allowed to discuss
...
- If `Contract bigger-loop` is later proposed for execution, it must re-enter in
  this order:
