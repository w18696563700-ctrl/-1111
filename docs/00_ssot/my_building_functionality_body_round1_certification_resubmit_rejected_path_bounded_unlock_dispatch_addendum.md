...
owner: Codex 总控
status: frozen
purpose: Freeze the bounded unlock dispatch for `certification/resubmit rejected-path success sample` inside `我的楼功能本体 Round 1`, rectifying the earlier dispatch freeze that still treated certification resubmit success-sample work as frozen after the current rejected-path runtime sample had already been materially established.
layer: L0 SSOT
...
# 《我的楼功能本体 Round 1 certification/resubmit rejected-path bounded unlock 派工补充单》
...
- 但当前 rejected-path 的实现包已经形成可独立引用的真实 runtime sample：
  - `Server` 已 materialize reviewer-side reject 与 profile-side resubmit canonical paths
  - 当前验证环境已真实跑通：
    - `reject -> resubmit`
  - reject 后 truth 已真实落为：
    - `rejected`
    - `rejectReason` 已写入
  - resubmit 后 truth 已真实回到：
    ...
    - `submittedAt` 已刷新
- 因此当前阻断不再是：
  - rejected-path runtime sample 不存在
- 而是：
  - 活跃 L0 派工边界尚未正式重开 `rejected-path certification/resubmit success sample`

## 3. Formal Rectification

- 自本补充单起，`我的楼功能本体 Round 1` 当前允许新增一个正式 bounded package：
  - `certification/resubmit rejected-path bounded unlock`
- 当前对该包的正式定义改为：
  - 不再是纯冻结占位
  - 已允许作为 `认证与成员身份 / 我的公司 / 我的楼 hub` 下的最小真实消费链
- 当前允许的最小消费范围只限：
  - 一个已真实成立的 `rejected` certification truth
...
- 当前 `certification/resubmit` 只允许真实承接：
  - `rejected-path` success sample
  - `rejectReason` 的最小 projection
...
- 本补充单只重开：
  - `rejected-path`
- 其它边界继续不变：
  - `expired-path` success sample 继续冻结
...
- 在 `certification/resubmit rejected-path bounded unlock` 正式重开后，当前仍未完成但继续冻结的只剩：
...
- 当前不再允许把：
  - `certification/resubmit rejected-path success sample`
 继续写成：
  - “当前继续冻结”
  - “当前不得推进”
...
- 当前 dispatch recommendation：
  - `Go` for `certification/resubmit rejected-path bounded unlock` inside `我的楼功能本体 Round 1`
- 其真实含义只有：
  - 本轮结果校验不得再把 rejected-path resubmit 实现本身判成“越权施工”
  - 后续增量推进可以把 rejected-path 当作当前已正式重开的 bounded package
- 其不代表：
...
- 下一轮唯一动作：
  - 输出《我的楼功能本体 Round 1 BFF 派工口令：certification/resubmit live success sample app-facing 对齐》
