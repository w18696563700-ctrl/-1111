...
status: frozen
purpose: >
  Freeze the refreshed bounded implementation dispatch bundle for
  `订单承接与履约承接主链` so post-cleanup execution authoring stays inside
  the already-frozen refreshed truth, contract, backend, BFF, and frontend
  chain without issuing any real implementation prompt yet.
layer: L0 SSOT
...
# 《订单承接与履约承接主链 refreshed bounded implementation dispatch bundle》
...
- 本派工包只适用于：
  - `订单承接与履约承接主链`
- 本派工包只冻结：
  - 当前 refreshed 实现轮的唯一目标
  - 当前允许的增量范围
  - 当前执行角色与顺序
  - 当前 allowed directories
...
- 当前实现轮唯一目标是：
  - 只把 post-cleanup 的 `order continuation read corridor + fulfillment continuation read corridor + milestone/submit / inspection/submit shell-handoff + workbench continuation carrier`
    做成 refreshed bounded implementation authoring 包
- 当前主链仅包括：
...
- `apps/server` 当前只允许实现 authoring：
  - `trading_read_corridor`
    内对以下最小 continuation read truth 的闭合：
    - `order/detail`
    - `contract/detail`
    - `milestone/list`
    - `inspection/detail`
  - `trading_shell_handoff`
    内对以下最小 shell / handoff truth position 的 authoring：
    - `milestone/submit`
    - `inspection/submit`
  - `exhibition_workbench`
    对 `activeOrderId / activeMilestoneId`
    的最小 continuation projection
  - `upload`
    对 `evidence / file_asset`
    的三段式复用，不改变 `objectKey`
    非真值原则
- `apps/server` 当前不允许实现：
  - `order/create`
  - `contract/confirm`
  ...
  - 新 path family
  - 把 `dispute/open`
    写回当前对象 canonical truth family
  - 沿用旧版过宽字段要求：
    - `milestone list -> sequenceNo` 本地必备
    - `inspection detail -> rectificationCount / recheckCount` 本地必备

### 3.2 BFF Included Scope

- `apps/bff` 当前只允许实现 authoring：
  - `trading_read_corridor`
    的最小 app-facing shaping：
    - `order/detail`
    - `contract/detail`
    - `milestone/list`
    - `inspection/detail`
  - `trading_shell_handoff`
    对以下节点的最小 command forward /
    controlled failure normalization：
    - `milestone/submit`
    - `inspection/submit`
  - `exhibition_workbench`
    对 `order_chain / fulfillment_chain`
    的最小 summary / handoff shaping
  - `file`
    route family 对 upload 三段式的既有复用承接
- `apps/bff` 当前不允许实现：
  - 第二套业务真义
  - 第二套状态机
  - 新 app-facing path
  - 把 `dispute/open`
    写回当前对象 included app-facing family
  - 把 `workbench / my-project`
    写成 truth owner
  - 本地推导：
    - `pass`
    - `complete`
...
- `apps/mobile` 当前只允许实现 authoring：
  ...
  - `/exhibition/milestones/submit`
    的最小 shell / handoff 消费
  - `/exhibition/inspections/detail`
    的最小只读消费
  - `/exhibition/inspections/submit`
    的最小 shell / handoff 消费
  - `workbench`
    的最小 continuation handoff 承接
- `apps/mobile` 当前不允许实现：
  - `my_project` detail owner 改造
  - `dispute_open_page`
  ...
  - 全站 UI 重做
  - 直连 `Server`
  - 把 page shell / placeholder / route presence
    写成 runtime 已通
  - 把以下字段重新写成本地必备：
    - `milestone list -> sequenceNo`
    - `inspection detail -> rectificationCount / recheckCount`
...
- frontend dispatch 后续只允许触达：
  ...
  - `apps/mobile/lib/features/exhibition/**`
    中与上述页面和命令直接相关的最小 consumer / supporting touch
- 当前不得放开：
...
1. `后端 Agent`
   - 先补齐 post-cleanup 的最小 continuation read truth、
     shell / handoff truth position、workbench continuation anchor projection、
     以及 upload evidence reuse
2. `BFF Agent`
   - 再补齐 post-cleanup 的最小 app-facing shaping、
     command forward、controlled failure normalization、
     以及 workbench handoff shaping
3. `前端 Agent`
   - 再补齐详情页 / 列表页 / 提交页
     refreshed bounded consumption closure
4. `结果校验 Agent`
   - 独立复核：
...
5. `联调发布 Agent`
   - 仅在前四步都通过后，
     重新给出当前对象的 integration judgment

## 7. Mandatory Receipt Rule

- 当前实现轮继续强制遵守：
  - backend / BFF / frontend 三份回执缺任一项，
    不启动结果校验
- 含义保持不变：
  - 后端 / BFF 回执允许先落云端
  - 前端回执先落本仓库 `docs/**`
...
- 不做 `my-project` 新 detail owner
- 不做 `workbench` 真值 owner 化
- 不做新 path family
- 不做第二状态机
- 不做 `objectKey` 真值化
- 不做 `dispute/open`
  refreshed implementation bundle authoring

## 9. Formal Conclusion

- 当前正式结论如下：
  - `订单承接与履约承接主链 = 当前可进入 refreshed backend implementation dispatch authoring`
  - `Phase 0 implementation exception unlock = 仍然 No-Go`
  - `real backend implementation dispatch issuance = 仍然 No-Go`
  - `direct implementation = 仍然 No-Go`
  - `integration = 仍然 No-Go`
  - `release-prep = 仍然 No-Go`
  - `production release = 仍然 No-Go`

## 10. Next Unique Action

- 下一步唯一动作：
  - 输出《订单承接与履约承接主链 refreshed backend implementation dispatch》
