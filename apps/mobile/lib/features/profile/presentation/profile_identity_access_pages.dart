...
enum _LoginEntryMode { otp, password }
...
class _LoginEntryPageState extends State<LoginEntryPage> {
  ...
  _LoginEntryMode _mode = _LoginEntryMode.otp;
  bool _sending = false;
  ...
  void _selectMode(_LoginEntryMode mode) {
    if (_mode == mode) {
      return;
    }
    setState(() {
    ...
    return ListView(
      ...
      children: <Widget>[
        const _IdentityHeroCard(
          title: '登录入口',
          summary:
              '当前入口只承接手机号验证码登录与账号密码登录。验证码登录在首次验证成功时会自动建号并直接登录，无需继续注册；登录完成后回到展览首页，不强跳公司与组织；其他能力继续按各自组织上下文受控。',
        ),
          ...
          child: AnimatedBuilder(
            ...
            builder: (BuildContext context, Widget? child) {
              ...
              return Column(
                ...
                children: <Widget>[
                  SegmentedButton<_LoginEntryMode>(
                    segments: const <ButtonSegment<_LoginEntryMode>>[
                      ButtonSegment<_LoginEntryMode>(
                        value: _LoginEntryMode.otp,
                        label: Text('验证码登录'),
                      ),
                      ButtonSegment<_LoginEntryMode>(
                        value: _LoginEntryMode.password,
                        label: Text('账号密码登录'),
                      ),
                    ],
                    selected: <_LoginEntryMode>{_mode},
                    onSelectionChanged: (Set<_LoginEntryMode> selection) {
                      _selectMode(selection.first);
                    },
                  ),
                  const SizedBox(height: 16),
                  if (_mode == _LoginEntryMode.otp)
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
                        TextField(
                        ...
                        Wrap(
                          ...
                          children: <Widget>[
                            ...
                            FilledButton.tonal(
                              onPressed:
                                  !_agreedToLegal || _sending || _loggingIn
                                  ? null
                                  : _loginWithOtp,
                              child: Text(_loggingIn ? '登录中' : '验证码登录'),
                            ),
                          ],
                        ),
                        const SizedBox(height: 12),
                        const Text(
                          '验证码登录在首次验证成功时会自动创建账号并直接登录，无需继续注册；登录成功后返回展览首页，不再强跳公司与组织。',
                    ...
                    Column(
                      ...
                      children: <Widget>[
                          ...
                          children: <Widget>[
                            FilledButton.tonal(
                              onPressed:
                                  ...
                                  ? null
                                  : _loginWithPassword,
                              child: Text(_loggingIn ? '登录中' : '密码登录'),
                            ),
                            TextButton(
                        ...
                        const SizedBox(height: 12),
                        const Text(
                          '账号密码登录只承接已补齐密码的手机号账号；若当前账号尚未设置密码，请先通过验证码登录后补齐密码。',
                          style: TextStyle(fontSize: 12, color: Colors.black54),
                        ),
                      ],
                    ),
                  const SizedBox(height: 12),
                  LoginLegalEntryStrip(
        ...
        if (loginResult != null) ...<Widget>[
          ...
          _IdentityStateCard(
            title: loginResult.state == AppPageState.content
                ? '登录已进入壳层承接'
                : authActionFailureTitle(loginResult, kind: _loginResultKind),
            message: loginResult.state == AppPageState.content
                ? authActionSuccessMessageForShell(loginResult.data!)
                : authActionFailureMessage(loginResult, kind: _loginResultKind),
          ),
        ],
      ],
    );
