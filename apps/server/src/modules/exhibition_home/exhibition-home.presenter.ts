...
export class ExhibitionHomePresenter {
  toReadModel(aggregation: ExhibitionHomeAggregationView) {
    const { location, weather } = aggregation;
    ...
    return {
      ...
      constructionRiskLevel: weather.constructionRiskLevel,
      constructionRiskSummary: weather.constructionRiskSummary,
      riskTags: weather.riskTags,
      riskTimeLabel: weather.riskTimeLabel,
      nightRainExpected: weather.nightRainExpected,
      nightRainTimeLabel: weather.nightRainTimeLabel,
      officialAlerts: weather.officialAlerts,
      constructionSuggestions: weather.constructionSuggestions,
      hourlyForecast: weather.hourlyForecast,
      dailyForecast: weather.dailyForecast,
      updatedAt: weather.updatedAt,
      sourceLabel: this.toSourceLabel(location.source, weather.state),
  ...
  private toSourceLabel(source: HomeLocationSource, weatherState: 'live' | 'degraded') {
    const weatherLabel =
      weatherState === 'live' ? '聚合真实天气' : '返回天气受控降级';
    switch (source) {
      case 'manual_selection':
        return `当前首页按手动选择地区${weatherLabel}`;
      case 'device_location':
        return `当前首页按定位地区${weatherLabel}`;
      case 'system_default':
        return `当前首页按系统默认地区${weatherLabel}`;
    }
  }
