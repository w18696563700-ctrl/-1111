...
type PaymentBillingReadAction = 'status' | 'explanation' | 'handoff';

@Injectable()
export class ProfilePaymentBillingStatusErrorService {
  ...
  private normalizeReadError(
    error: unknown,
    action: PaymentBillingReadAction,
    fallbackCode: string,
  ...
  private normalizeCode(code: string, originalMessage: string) {
    if (originalMessage.startsWith('Cannot GET /server/profile/payment-and-billing-status/')) {
      return 'PAYMENT_AND_BILLING_STATUS_ROUTE_UNAVAILABLE';
    }
    ...
    if (
      ...
      code === 'PAYMENT_AND_BILLING_STATUS_ROUTE_UNAVAILABLE' ||
  ...
  private translateMessage(action: PaymentBillingReadAction, code: string) {
    if (code === 'AUTH_SESSION_INVALID') {
      return '当前登录态不可用，请重新登录或刷新后再试。';
    }
    if (code === 'AUTH_PERMISSION_INSUFFICIENT') {
      return '当前无权限查看支付与账单状态。';
    }
    if (code === 'AUTH_RESOURCE_UNAVAILABLE') {
      return '当前支付与账单资源不可用，请稍后再试。';
    }
    if (code === 'PAYMENT_AND_BILLING_STATUS_ROUTE_UNAVAILABLE') {
      return '当前支付与账单入口暂不可用，请稍后再试。';
    }
    if (code === 'PAYMENT_STATUS_UNAVAILABLE') {
    ...
    if (code === 'BILLING_REFERENCE_UNAVAILABLE') {
      return action === 'explanation'
        ? '当前账单引用说明暂不可用，请稍后再试。'
        : action === 'handoff'
          ? '当前账单引用引导暂不可用，请稍后再试。'
    ...
    if (code === 'PAYMENT_HANDOFF_UNAVAILABLE') {
      return action === 'explanation'
        ? '当前支付处理说明暂不可用，请稍后再试。'
        : '当前支付处理引导暂不可用，请稍后再试。';
    }
